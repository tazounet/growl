...
+(SMTPClient*)clientWithServerAddress:(NSString*)address 
										  ports:(NSArray*)ports 
...
-(void)handleData:(NSMutableData*)data {
	...
	while (datal > 0) {
		...
		if (l) {
			NSString* line = [[NSString alloc] initWithBytesNoCopy:datap 
																			length:l 
			...
			switch (code) {
				case 250:
					if (separator == '-') {
						NSString* name;
						NSString* value;
						[message splitStringAtCharacterFromSet:NSCharacterSet.whitespaceCharacterSet 
															 intoChunks:&name:&value 
															  separator:NULL];
