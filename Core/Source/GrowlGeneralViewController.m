...
-(instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil forPrefPane:(GrowlPreferencePane *)aPrefPane {
   if((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil forPrefPane:aPrefPane])){
      ...
      self.iconMenuOptionsList = @[NSLocalizedString(@"Show icon in the menubar", @"Growl will have a menu bar icon only"),
                                   ...
                                   NSLocalizedString(@"No icon visible", @"Growl will run compeletely in the background")];
...
-(void)awakeFromNib
{
   startAtLoginSwitch.state = (self.preferencesController).shouldStartGrowlAtLogin;
   [startAtLoginSwitch addObserver:self 
                        forKeyPath:@"state" 
                           options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld
                           context:nil];
   
   useAppleNotificationCenterSwitch.state = (self.preferencesController).shouldUseAppleNotifications;
   ...
   growlClawLogo.hidden = hasNotificationCenter;
   useAppleNotificationCenterSwitch.hidden = !hasNotificationCenter;
   useAppleNotificationCenterLabelField.hidden = !hasNotificationCenter;
	useAppleNotificationCenterExplanationField.hidden = !hasNotificationCenter;
   ...
   if (!hasNotificationCenter) {
	...
	self.showRulesUI = [GrowlUserScriptTaskUtilities hasRulesScript];
	[[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)updateRulesState:(NSNotification*)notification {
	self.showRulesUI = [[notification.userInfo valueForKey:@"hasRules"] boolValue];
}
...
-(IBAction)startGrowlAtLogin:(id)sender{
   if(startAtLoginSwitch.state){
      if(!(self.preferencesController).allowStartAtLogin){
          ...
          [alert setMessageText:NSLocalizedString(@"Alert! Enabling this option will add Growl.app to your login items", nil)];
          [alert addButtonWithTitle:NSLocalizedString(@"Ok", nil)];
          [alert addButtonWithTitle:NSLocalizedString(@"Cancel", nil)];
          [alert setInformativeText:NSLocalizedString(@"Allowing this will let Growl launch everytime you login, so that it is available for applications which use it at all times", nil)];

          [alert beginSheetModalForWindow:[sender window] completionHandler:^(NSModalResponse returnCode) {
              [self startGrowlAtLoginAlert:returnCode];
          }];
      }else{
...
{
   (self.preferencesController).shouldUseAppleNotifications = useAppleNotificationCenterSwitch.state;
}
...
- (IBAction)startGrowlAtLoginAlert:(NSModalResponse)returnCode
{
   switch (returnCode) {
