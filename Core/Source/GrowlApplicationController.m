   ...
   NSDictionary *growlNotificationDict = [self.growlDicts valueForKey:noteKey];
	if(growlNotificationDict){
      ...
      if(notification.activationType == NSUserNotificationActivationTypeActionButtonClicked) {
         ...
         temporaryNotificationDict[GROWL_NOTIFICATION_CLICK_BUTTONUSED] = @YES;
      }
...
- (void) showPreview:(NSNotification *) note {
	@autoreleasepool {
		...
		if([displayConfig isKindOfClass:[NSSet class]]){
			...
			NSNumber *priority = @0;
			NSNumber *sticky = @NO;
			NSDictionary *info = @{GROWL_APP_NAME: @"Growl",
                                   GROWL_NOTIFICATION_NAME: @"Preview",
                                   GROWL_NOTIFICATION_TITLE: NSLocalizedString(@"Preview", "Title of the Preview notification shown to demonstrate Growl displays"),
                                   GROWL_NOTIFICATION_DESCRIPTION: desc,
                                   GROWL_NOTIFICATION_PRIORITY: priority,
                                   GROWL_NOTIFICATION_STICKY: sticky,
                                   GROWL_NOTIFICATION_ICON_DATA: [NSImage imageNamed:NSImageNameApplicationIcon]};
         ...
         if(!configCopy)
			...
			[configCopy setValue:@([GrowlPreferencesController sharedController].selectedPosition)
...
- (void) _fireAppleNotificationCenter:(NSDictionary *)growlDict
{
   ...
   if ([growlDict[GROWL_NOTIFICATION_ALREADY_SHOWN] boolValue])
      return;
   ...
   //If the note came from a 3.0 framework, and it hasn't already been displayed, tell it to display
	...
	if(!dispatchedToBark){
		...
		appleNotification.title = growlDict[GROWL_APP_NAME];
		appleNotification.subtitle = growlDict[GROWL_NOTIFICATION_TITLE];
		appleNotification.informativeText = growlDict[GROWL_NOTIFICATION_DESCRIPTION];
		
		NSString *noteKey = [NSProcessInfo processInfo].globallyUniqueString;
		appleNotification.userInfo = @{@"AppleNotificationID": noteKey};
		appleNotification.hasActionButton = NO;
		
		if (growlDict[GROWL_NOTIFICATION_BUTTONTITLE_ACTION]) {
			appleNotification.hasActionButton = YES;
			appleNotification.actionButtonTitle = growlDict[GROWL_NOTIFICATION_BUTTONTITLE_ACTION];
		}
		
		if (growlDict[GROWL_NOTIFICATION_BUTTONTITLE_CANCEL])
			appleNotification.otherButtonTitle = growlDict[GROWL_NOTIFICATION_BUTTONTITLE_CANCEL];
		
		static dispatch_once_t onceToken;
		dispatch_once(&onceToken, ^{
			appleNotificationDelegate = [[GrowlApplicationNotificationCenterDelegate alloc] init];
		});
		(appleNotificationDelegate.growlDicts)[noteKey] = notificationDict;
		
		[NSUserNotificationCenter defaultUserNotificationCenter].delegate = appleNotificationDelegate;
                                    ...
                                    if([result descriptorForKeyword:'NtRt']){
														...
														priority != GrowlPriorityUnset)
                                       {
                                          if(title && ![title isEqualToString:[copyDict valueForKey:GROWL_NOTIFICATION_TITLE]]){
                                             mutableCopy[GROWL_NOTIFICATION_TITLE] = title;
                                             changed = YES;
                                             if(logRuleResult){
                                          ...
                                          if(description && ![description isEqualToString:[copyDict valueForKey:GROWL_NOTIFICATION_DESCRIPTION]]){
                                             mutableCopy[GROWL_NOTIFICATION_DESCRIPTION] = description;
                                             changed = YES;
                                             if(logRuleResult){
                                          ...
                                          if(iconData && ![iconData isEqualToData:[copyDict valueForKey:GROWL_NOTIFICATION_ICON_DATA]]){
                                             ...
                                             if(imageFromData != nil){
                                                mutableCopy[GROWL_NOTIFICATION_ICON_DATA] = iconData;
                                                changed = YES;
                                                if(logRuleResult){
                                                   [ruleLogString appendFormat:@"\nModified icon"];
                                                }
                                          ...
                                          if(sticky && sticky.booleanValue != [[copyDict valueForKey:GROWL_NOTIFICATION_STICKY] boolValue]){
                                             mutableCopy[GROWL_NOTIFICATION_STICKY] = [NSNumber numberWithBool:sticky.booleanValue];
                                             changed = YES;
                                             if(logRuleResult){
                                                [ruleLogString appendFormat:@"\nModified sticky to: %@", sticky ? @"YES" : @"NO"];
														...
														if(priority != GrowlPriorityUnset && priority != [[copyDict valueForKey:GROWL_NOTIFICATION_PRIORITY] integerValue]){
															changed = YES;
															mutableCopy[GROWL_NOTIFICATION_STICKY] = @(priority);
															if(logRuleResult){
...
-(GrowlTicketDatabaseApplication*)appTicketForDict:(NSDictionary*)dict {
   NSString *appName = dict[GROWL_APP_NAME];
   NSString *hostName = dict[GROWL_NOTIFICATION_GNTP_SENT_BY];
...
-(GrowlTicketDatabaseNotification*)notificationTicketForDict:(NSDictionary*)dict {
   NSString *notificationName = dict[GROWL_NOTIFICATION_NAME];   
...
-(NSDictionary*)filledInNotificationDictForDict:(NSDictionary*)dict
{
   ...
   NSData *iconData = nil;
   id sourceIconData = aDict[GROWL_NOTIFICATION_ICON_DATA];
   if (sourceIconData) {
   ...
   if (iconData)
      aDict[GROWL_NOTIFICATION_ICON_DATA] = iconData;
   else{
      ...
      aDict[GROWL_NOTIFICATION_ICON_DATA] = defaultIconData;
   }
   
   // If app icon present, convert to NSImage
   iconData = nil;
   sourceIconData = aDict[GROWL_NOTIFICATION_APP_ICON_DATA];
   if (sourceIconData) {
      if ([sourceIconData isKindOfClass:NSImageClass])
         iconData = ((NSImage *)sourceIconData).PNGRepresentation;
      else if ([sourceIconData isKindOfClass:NSDataClass])
   ...
   if (iconData)
      aDict[GROWL_NOTIFICATION_APP_ICON_DATA] = iconData;
   else{
      static NSData *defaultIconData = nil;
      static dispatch_once_t onceToken;
      dispatch_once(&onceToken, ^{
      ...
      aDict[GROWL_NOTIFICATION_APP_ICON_DATA] = defaultIconData;
   }
   
   // To avoid potential exceptions, make sure we have both text and title
   if (!aDict[GROWL_NOTIFICATION_DESCRIPTION])
      aDict[GROWL_NOTIFICATION_DESCRIPTION] = @"";
   if (!aDict[GROWL_NOTIFICATION_TITLE])
      aDict[GROWL_NOTIFICATION_TITLE] = @"";
   ...
   if (priority == GrowlPriorityUnset) {
      value = dict[GROWL_NOTIFICATION_PRIORITY];
      if (!value)
         value = @(GrowlPriorityNormal);
   } else
      value = @(priority);
   aDict[GROWL_NOTIFICATION_PRIORITY] = value;
      
   // Retrieve and set the sticky bit of the notification
   int sticky = notification.sticky.intValue;
   if (sticky >= 0)
      aDict[GROWL_NOTIFICATION_STICKY] = [NSNumber numberWithBool:sticky];
      
   /* Set a unique ID which we can use globally to identify this particular notification if it doesn't have one */
   if (!aDict[GROWL_NOTIFICATION_INTERNAL_ID]) {
...
- (BOOL) registerApplicationWithDictionary:(NSDictionary *)userInfo {
	NSString *appName = userInfo[GROWL_APP_NAME];
   if(!appName){
...
- (NSDictionary *) versionDictionary {
	if (!versionInfo) {
		...
		NSNumber *major = @(version.major);
		NSNumber *minor = @(version.minor);
		NSNumber *incremental = @(version.incremental);
		NSNumber *releaseType = @(version.releaseType);
		NSNumber *development = @(version.development);

		versionInfo = @{(NSString *)kCFBundleVersionKey: [GrowlApplicationController growlVersion],

			@"Major version": major,
			@"Minor version": minor,
			@"Incremental version": incremental,
			@"Release type name": releaseTypeNames[version.releaseType],
...
- (NSString *)stringWithVersionDictionary:(NSDictionary *)d {
	...
	//the .1 in 0.6.1
	NSNumber *incremental = d[@"Incremental version"];
	if (incremental.unsignedShortValue)
		[result appendFormat:@".%@", incremental];

	NSString *releaseTypeName = d[@"Release type name"];
	if (releaseTypeName.length) {
		//"" (release), "b4", " SVN 900"
		[result appendFormat:@"%@%@", releaseTypeName, d[@"Development version"]];
	}
...
- (IBAction)quitWithWarning:(id)sender
{
    if(![[NSUserDefaults standardUserDefaults] boolForKey:@"HideQuitWarning"])
    {
        ...
        [alert addButtonWithTitle:NSLocalizedString(@"Yes", nil)];
        [alert addButtonWithTitle:NSLocalizedString(@"No", nil)];
        [alert setInformativeText:NSLocalizedString(@"If you quit Growl you will no longer receive notifications.", nil)];
        alert.alertStyle = NSAlertStyleWarning;
...
- (void) toggleRollup
{
    ...
    [GrowlPreferencesController sharedController].rollupShown = show;
}
...
{   
   ...
   GrowlPreferencesController *preferences = [GrowlPreferencesController sharedController];
   if([components[0] caseInsensitiveCompare:@"preferences"] == NSOrderedSame){
      [self showPreferences];
      if(components.count > 1){
         NSString *tab = components[1];
         if([tab caseInsensitiveCompare:@"general"] == NSOrderedSame) {
            preferences.selectedPreferenceTab = 0;
         }else if([tab caseInsensitiveCompare:@"applications"] == NSOrderedSame){
            preferences.selectedPreferenceTab = 1;
            if(components.count > 2){
               NSString *app = components[2];
               NSString *host = nil;
               NSString *note = nil;
               if(components.count > 3 && ![components[3] isEqualToString:@""])
                  host = components[3];
               if(components.count > 4 && ![components[4] isEqualToString:@""])
                  note = components[4];
            ...
            preferences.selectedPreferenceTab = 2;
            if(components.count > 2){
               NSString *display = components[2];
            ...
            preferences.selectedPreferenceTab = 3;
            if(components.count > 2){
               NSString *forwardSubscribe = components[2];
            ...
            preferences.selectedPreferenceTab = 4;
            ...
            preferences.selectedPreferenceTab = 6;
         }
      }
   }else if([components[0] caseInsensitiveCompare:@"plugin"] == NSOrderedSame) {
		if(components.count > 1){
			if([components[1] caseInsensitiveCompare:@"preview"] == NSOrderedSame) {
				if(components.count > 2){
					NSString *pluginPreviewString = components[2];
...
- (void)handleGetURLEvent:(NSAppleEventDescriptor *)event withReplyEvent:(NSAppleEventDescriptor *)replyEvent
{
   ...
   NSString *escaped = [url stringByAddingPercentEncodingWithAllowedCharacters:set];
...
- (void) applicationWillFinishLaunching:(NSNotification *)aNotification {
	...
	destDir = searchPath[0U]; //first == last == ~/Library
...
- (void)expiryCheck
{
    if([self expired])
    {
        [NSApp activateIgnoringOtherApps:YES];
        NSAlert *alert = [[NSAlert alloc] init];
        [alert setMessageText:@"This Beta Has Expired"];
...
- (void) checkForCorruption
{
    ...
    for(NSString *component in [NSBundle mainBundle].bundlePath.pathComponents)
    {
        ...
        {
            NSAlert *alert = [[NSAlert alloc] init];
            [alert setMessageText:NSLocalizedString(@"Corrupt install detected", @"we've detected a corrupt install in this currently running binary")];
...
- (void) applicationDidFinishLaunching:(NSNotification *)aNotification {
   ...
   [dnc addObserver:self
	...
	growlNotificationCenterConnection = [[NSConnection alloc] init];
	//[growlNotificationCenterConnection enableMultipleThreads];
	growlNotificationCenterConnection.rootObject = growlNotificationCenter;
...
- (BOOL) applicationShouldHandleReopen:(NSApplication *)theApplication hasVisibleWindows:(BOOL)flag {
   ...
   if(db.notificationsWhileAway && !db.historyWindow.window.visible)
               ...
               onLocalMachine:(BOOL)wasLocal
{
   ...
   id callbackTarget = growlNotificationDict[GROWL_NOTIFICATION_CALLBACK_URL_TARGET];
   ...
   [self sendNotificationDict:growlNotificationDict feedbackOfType:noteName];
	...
	if (!wasLocal) {
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:GROWL_CLOSE_NOTIFICATION
															object:growlNotificationDict[GROWL_NOTIFICATION_INTERNAL_ID]];
...
{
   NSString *gntpOrigin = growlNotificationDict[GROWL_GNTP_ORIGIN_SOFTWARE_NAME];
   if([gntpOrigin caseInsensitiveCompare:@"Growl.framework"] == NSOrderedSame &&
      [growlNotificationDict[GROWL_NOTIFICATION_GNTP_SENT_BY] isLocalHost]){
      NSString *frameworkVersion = growlNotificationDict[GROWL_GNTP_ORIGIN_SOFTWARE_VERSION];
      if(compareVersionStrings(@"3.0", frameworkVersion) != kCFCompareGreaterThan){
         NSString *noteUUID = growlNotificationDict[GROWL_NOTIFICATION_INTERNAL_ID];
